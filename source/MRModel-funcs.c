...
/*   int *drw = malloc(*tm_ht * sizeof(int)); */
...
/*     /\*do the multinomial draw...if tm_ht is in columns then */
/*      we do not need to do any vector copying. */

/*     Aslo assumes that only forward transitions are possible*\/ */
/*     rmultinom(curstate[i], &transmatrix[i * *tm_ht], *tm_ht, drw); */

/*     /\*update the new state...assumes state forwardness*\/ */
/*     for (j=i;j<*len;j++) { */
/*       newstate[j] = newstate[j]+drw[j]; */
/*     } */
