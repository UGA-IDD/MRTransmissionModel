...
rcv_exp <- EX.Country.part2(uncode = setup$uncode,
                                  generation.time = 0.5, 
                                  ...
                                  year=1980,
                                  EXt0=EXt0,
                                  time.specific.MR1cov =  setup$RCV1.coverage.1980to2100, 
                                  time.specific.MR2cov = rep(0, length(setup$RCV1.coverage.1980to2100)),  
                                  time.specific.SIAcov =  setup$rubellaSIA.coverage.1980to2100, 
...
plot(rcv_exp@result)
tmpr <- rcv_exp
#Get output and storing into into output_vacc_exp
output_vacc_exp <- getOutput_201910gavi_v4(sim.res=tmpr, t.max=t.max, setup=setup, year=year, 
                                            ...
                                            scenario="vacc experiment", run.number=s, R0=R0)




###################### Example Montagu (i.e., VIMC) data experiment run ########################
detach("package:wpp2019", unload=TRUE)
library(wpp2017) #VIMC model inputs require wpp2017 
