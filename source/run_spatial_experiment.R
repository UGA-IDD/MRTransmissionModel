...
## Or load the package from the Desktop - but not working b/c need revised functions that have been changd in the base.R file
#install.packages("~/Desktop/MRTransmissionModel_1.0.0.tar.gz", repos = NULL, type = "source")
#library(MRTransmissionModel)

## New / Revised functions that need to be loaded
source("./source/new_spatial_functions.R")
source("./source/new_spatial_functions.R")
source("./source/revised_functions.R")
source("./source/spatial_user_interface.R")
source("./source/spatial_inputs.R")
...
setup$asdr.object <- space.nMx <- new("space.nMx",
                                      ...
                                      n.subpops = 2)
#setup <- space.setupCountry("Zambia")
