...
#' @export
#'
setupCountry.Sep2026 <- function(country){

  year_of_last_vaccination_data <- 2025
  ...
  demog <- getDemography.wpp2024(uncode=uncode)
  ...
  filep <- system.file("/extdata/MCV_WUENIC_coverage_estimates_DownloadedJuly2026.xlsx",
                       package = "MRTransmissionModel")
  #filep <- "inst/extdata/MCV_WUENIC_coverage_estimates_DownloadedJuly2026.xlsx" #use prior to rebuilding package
  ...
  MCV1.coverage <- df %>%
    ...
    dplyr::mutate(COVERAGE=COVERAGE/100) %>%
  ...
  filep2 <- system.file("/extdata/RCV_WUENIC_coverage_estimates_DownloadedJuly2026.xlsx",
                        package = "MRTransmissionModel")
  #filep2 <- "inst/extdata/RCV_WUENIC_coverage_estimates_DownloadedJuly2026.xlsx" #use prior to rebuilding package
  ...
  SIA.measles <- getWHO.SIAs.Sep2026(iso3code, uncode, upper.limit=0.97, demog)
  ...
  SIA.rubella <- getWHO.SIAs.Sep2026(iso3code, uncode, upper.limit=0.97, demog, rubella.SIAs=TRUE)
  ...
  get.births.here <- function(state,tran,
                              fert.curve=fert,
                              lower.age.boundary = c(0,as.numeric(row.names(asfr.1950.2100)),50,101)) {
