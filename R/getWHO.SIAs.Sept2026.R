...
#' @param demog demography from setupCountry.Jun2026()
#' @param rubella.SIAs boolean - default FALSE means pulls measles SIAs, if TRUE then pulls rubella SIAs
#'
...
getWHO.SIAs.Sep2026 <- function(iso3code, uncode, upper.limit=0.97, demog, rubella.SIAs=FALSE){

  filep <- system.file("/extdata/SIAs_WHO_Downloaded4Sept2026.csv",
                       package = "MRTransmissionModel")
  #filep <- "inst/extdata/SIAs_WHO_Downloaded4Sept2026.csv" #use prior to rebuilding package
  df <- readr::read_csv(filep)

  # pulling rubella or measles SIAs
  if (rubella.SIAs){
    df <- df %>%
      dplyr::filter(grepl("RCV|MR", INTERVENTION, ignore.case = TRUE))
  } else {
    df <- df %>%
      dplyr::filter(grepl("MCV|measles|MR", INTERVENTION, ignore.case = TRUE))
  }

  # must be done and not missing neccessary data
  df <- df %>%
  ...
  #keep only preventative SIAs
  df <- ConvertActivitySIA(df_sia=df, PIRI=FALSE, OR=FALSE, OTHER=FALSE)

  # filter to only the country of interest
  mtch <- which(df$COUNTRY==iso3code,arr.ind=TRUE)
  df <- df[mtch,]

  if (nrow(df)!=0){

    age.range.new <- ConvertAgeSIA_v2(sia.age.range = df$AGEGROUP)
    ...
    for (s in 1:length(years.sia.new)){
      ...
      if (is.na(age.min) | is.na(age.max)){
        ...
        if (df$EXTENT[s]=="national" | df$EXTENT[s]=="National" | df$EXTENT[s]=="NATIONAL"){
          if (!is.na(df$SURVEY_RESULT[s])){
            coverage.new[s] <- (df$SURVEY_RESULT[s])/100
          } else {
            coverage.new[s] <- (df$ADMIN_COVERAGE[s])/100
          }
        } else { #Extent is Rollover-National or Subnational
          #get national population of the target age
          uncode <- countrycode(df$COUNTRY[s], "iso3c", "un")
          out <- demog
          if (age.min==1){
          ...
          if (!is.na(df$SURVEY_RESULT[s])){
            coverage.new[s] <- (df$SURVEY_RESULT[s])/100 * (df$TARGET[s]) / tot.pop
          } else {
            coverage.new[s] <- (df$DOSES[s]) / tot.pop
          }
        }
